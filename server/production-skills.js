...
const references = {
    ...
    audio: [upstream('game-audio'), 'agency-agents/game-audio-engineer.md'],
...
const specs = [
    ...
    ['animation', ['character'], 'specification', '规范已固定，执行入口待接入', ['角色模型、骨架、引擎与动作清单'], ['骨架、动作状态与事件规格；执行后交付动画']],
    ['audio', ['narrative', 'environment'], 'specification', '规范已固定，生成入口待接入', ['声源、触发条件、时长、循环与情绪'], ['声音事件表、生产提示词；生成后交付音频']],
    ['integration', ['character', 'environment', 'prop', 'animation', 'audio'], 'external', '需在游戏工程执行', ['已验收资产、引擎和平台约束'], ['可玩切片、资产映射和性能记录']],
    ['qa', ['integration'], 'human', '需实际试玩和人工验收', ['可玩构建、目标设备、验收用例'], ['问题与复测证据、发布资产清单']]
];
const skills = Object.fromEntries(Object.keys(names).map(key => {
    const content = fs.readFileSync(path.join(root, key, 'SKILL.md'), 'utf8').replace(/^---\r?\n[\s\S]*?\r?\n---\r?\n/, '').trim();
    ...
    return [key, { id: `production-${key}`, name: names[key], version: 1, content, sources, production: true }];
}));
