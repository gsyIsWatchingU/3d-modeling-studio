...
test('引导式工位支持独立生成、依赖交接、预览图后端保存和人工审核', { timeout: 40000 }, async t => {
    ...
    const planner = http.createServer(async (req, res) => {
        ...
        const body = JSON.parse(raw); plannerRequests++;
    ...
    const finalProject = await data(request(`/projects/${project.id}`));
    assert.equal(finalProject.stage_runs.at(-1).review.status, 'approved'); assert.equal(plannerRequests, 3);
});
